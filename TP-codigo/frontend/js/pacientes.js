let pacientesData = []; // guarda la última lista traída del servidor, para filtrar sin volver a pedirla
...
function filtrarPacientes() {
  ...
  const filtrados = !termino
    ? pacientesData
    : pacientesData.filter(p => {
        const obraSocial = p.obraSocial?.nombreObra || "";
        return (
          p.nombre?.toLowerCase().includes(termino) ||
          p.apellido?.toLowerCase().includes(termino) ||
          p.dni?.toString().toLowerCase().includes(termino) ||
          p.nombreUsuario?.toLowerCase().includes(termino) ||
          obraSocial.toLowerCase().includes(termino)
        );
      });
