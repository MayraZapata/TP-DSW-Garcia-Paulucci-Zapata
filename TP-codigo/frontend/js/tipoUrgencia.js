let tiposUrgenciaData = []; // guarda la última lista traída del servidor, para filtrar sin volver a pedirla
...
function renderTiposUrgencia(tiposUrgencia) {
    ...
    if (tiposUrgencia.length === 0) {
        lista.innerHTML = "<li>No se encontraron tipos de urgencia.</li>";
        return;
    }
...
async function cargarTiposUrgencia() {
    ...
    tiposUrgenciaData = await respuesta.json();
